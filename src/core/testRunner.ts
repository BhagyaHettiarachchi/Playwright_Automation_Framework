...
export class AITestRunner {
  ...
  private async executeStepWithHealing(
    ...
    // ✅ FIX: Handle navigate and wait FIRST before finding any element
    // These actions don't need a locator/selector
    ...
    // Only call findElementWithHealing for actions that need an element
    const { locator, healed, newSelector } = await this.selfHealing.findElementWithHealing(
      page,
      selector,
      description
...
<body>
  <div class="container">
    <h1>🤖 AI-Augmented Test Execution Report</h1>
    ...
    ${this.results.map(result => `
      <div class="test-result ${result.status.toLowerCase()}">
        <h3>${result.status === 'PASSED' ? '✓' : '✗'} Test Case: ${result.testCaseId}</h3>
        <p><strong>Execution Time:</strong> ${(result.executionTime / 1000).toFixed(2)}s</p>
        ${result.selfHealingApplied ? '<p>🔧 Self-Healing Applied</p>' : ''}
